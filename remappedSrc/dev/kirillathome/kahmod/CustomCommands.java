...
public final class CustomCommands {
    ...
    public static int reloadConfig(ServerCommandSource source){
        ...
        for (ServerPlayerEntity player : source.getWorld().getPlayers()){
            player.networkHandler.send(new CustomPayloadS2CPacket(new BrandCustomPayload(ConfigManager.getServerConfig().customBrand)), PacketCallbacks.always(() -> Kahmod.LOGGER.info("i dunno")));
        }
