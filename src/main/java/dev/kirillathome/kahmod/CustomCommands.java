...
public final class CustomCommands {
    ...
    public static void registerRSMPCommand(CommandDispatcher<ServerCommandSource> dispatcher){
        dispatcher.register(CommandManager.literal("kahmod")
    ...
    public static int setStatus(ServerCommandSource source, String status, Formatting formatting){
        ...
        String player_name = source.getPlayer().getNameForScoreboard();
        ...
        source.getServer().getScoreboard().addScoreHolderToTeam(player_name, player_team);
        if (player_team == null){
            return 1;
        }
    ...
    public static int reloadConfig(ServerCommandSource source){
        ...
        for (ServerPlayerEntity player : source.getWorld().getPlayers()){
            player.networkHandler.send(new CustomPayloadS2CPacket(new BrandCustomPayload(config.customBrand)), PacketCallbacks.always(() -> {}));
        }
